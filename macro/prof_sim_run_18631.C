void prof_sim_run_18631() { 
  ...
  ofstream myfile("slopes_run_18631_bpmx.txt");
  if(!myfile.is_open()){
    cerr << "Error opening file!" << endl;
    return;
  }

  myfile << "run, bpm, magnet, slope, slope_err \n";
  ...
  for (int j=0; j<num_magnet; j++) {
    ...
    for (int i=0; i<num_bpm; i++) {
      ...
      myfile << (int)18631 << "  " << bpm[i].Data() << "  " << magnet[j].Data() << "  " << p1*pow(10,6) << "  " << p1_err << endl;
  ...
  gSystem->Exec("pdfunite $(ls -rt /adaqfs/home/apar/arindam/20250618_bmod_study/temp/*.pdf) /adaqfs/home/apar/arindam/20250618_bmod_study/magnet_prof_run_18631x.pdf");
  myfile.close();
}
